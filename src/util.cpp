...
const char * const BITCOIN_CONF_FILENAME = "exilium.conf";
const char * const BITCOIN_PID_FILENAME = "exiliumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "exilium" is a composite category enabling all exilium-related debug output
            if(ptrCategory->count(string("exilium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\exilium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\exilium
    // Mac: ~/Library/Application Support/exilium
    // Unix: ~/.exilium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "exilium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/exilium";
#else
    // Unix
    return pathRet / ".exilium";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty exilium.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override exilium.conf
