 ...
 *
 *   // network
 *   CexiliumNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.exilium.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CexiliumNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CexiliumNetwork(UniValue objIn)
    {
...
// // can be under: exiliumNetwork
// //   -- signature requirements : Key1(User)
// class CexiliumNetworkVariable : public CGovernanceObject
// {
// private:
